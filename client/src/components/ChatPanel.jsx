...
export default function ChatPanel({ myId, playerName }) {
  ...
  return (
    <div className="card-dark rounded-xl flex flex-col h-full overflow-hidden">
      {/* Header */}
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-1">
                {!isMe && (
                  <span className="text-gold-600 text-xs font-semibold">{msg.playerName}</span>
                )}
                <span className="text-gold-800 text-xs">{formatTime(msg.timestamp)}</span>
                {isMe && (
                  <span className="text-gold-500 text-xs font-semibold">You</span>
                )}
              </div>
              {/* Bubble */}
              <div
